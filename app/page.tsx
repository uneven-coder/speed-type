...
function renderCharacterComparison(functionalQuote: string[], typedText: string): JSX.Element[]
{   // Create character-by-character comparison elements
    ...
    while (quotePointer < functionalQuote.length && typedPointer < typedText.length)
...
{   // Main typing display component with character comparison
    ...
    const characterElements = renderCharacterComparison(functionalQuote, typedText);

    return (
        <p className="text-[var(--text)] text-6xl font-mono">
            {characterElements}
        </p>
    );
}

function useTypingInput(quote: string[], onReset: () => void): [string, string]
{   // Handle keyboard input and typing state management
    const [typedText, setTypedText] = useState<string>("");
    const [lastTyped, setLastTyped] = useState<string>("");

    const isValidChar = (key: string): boolean => /^[A-Za-z.']+$/.test(key) || key === " ";
...
{   // Main typing test application component
    ...
    {   // Reset to default quote for new test
        setQuote("Your problem is you spent your whole life thinking there are rules. There aren't.".split(" "));
    };
