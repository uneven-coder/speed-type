...
function saveFastestWpm(wpm: number): void {
    // Save fastest WPM to localStorage with error handling
...
function calculateWpm(typedText: string, startTime: number | null): number {
    // Calculate words per minute based on typed text and elapsed time
...
function handleScrollToActive(currentLine: number): void {
    // Handle scrolling to the active line
...
function updateSpanOpacity(currentLine: number): void {
    // Update opacity for all spans based on current line
...
function useScrollAndFade(currentLine: number): void {
    useEffect(() => {
        ...
        handleScrollToActive(currentLine);
        updateSpanOpacity(currentLine);
    }, [currentLine]);
}
...
function useViewportHeight(): void {
    ...
    useEffect(() => {
        ...
        function updateHeight(): void {
            ...
            if (isKeyboardVisible) document.documentElement.setAttribute("data-keyboard-visible", "true");
            else document.documentElement.removeAttribute("data-keyboard-visible");

            if (isSmallScreen) document.documentElement.setAttribute("data-small-screen", "true");
            else document.documentElement.removeAttribute("data-small-screen");
        }

        updateHeight();

        const handleResize = () => updateHeight();
        const handleScroll = () => updateHeight();
            ...
            <div className="grid grid-cols-3 gap-x-1 gap-y-0.5 sm:grid-cols-5 sm:gap-1.5 md:gap-2 lg:gap-4">
                ...
                <div className="flex flex-col items-center justify-center gap-0.5">
                    ...
                    <button
                        type="button"
                        onClick={toggleTheme}
                        className="rounded-md border-2 border-[var(--accent)] bg-[var(--background)] px-2 py-1 font-semibold text-[var(--text)] text-sm leading-none transition-colors hover:bg-[var(--accent)] hover:text-[var(--background)] md:rounded-lg md:border-4 md:px-4 md:py-2 md:text-base"
